...
export function TenderList({
  ...
  const handleStatusFilter = (status: string) => {
    ...
    // TODO: Implement status filtering in server action
    // For now, just filter client-side
    if (status === 'all') {
      fetchTenders(searchQuery, 1);
    } else {
      const filtered = initialTenders.filter((t) => t.status === status);
      setTenders(filtered);
  ...
  const handleDeleteTender = async (tenderId: string) => {
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        ...
        fetchTenders(searchQuery, currentPage);
      } else {
